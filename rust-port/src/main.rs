...
use openfused::{crypto, store, validity, ContextStore};
...
enum Commands {
    ...
    /// Start the HTTP daemon: serve context store over HTTP (A2A + peer sync)
    #[cfg(feature = "daemon")]
    Serve {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        #[cfg(feature = "daemon")]
        Commands::Serve { store, port, bind, public, token, gc_days } => {
